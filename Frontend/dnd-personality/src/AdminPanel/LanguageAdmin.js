...
const columns = [
    ...
    {field: 'name', headerName: 'Language', width: 150 }
];
...
const removeOnClick = async() => {
        if(selectionModel.length > 0){
            selectionModel.forEach(index => {
                fetch(apiURL + '/languages/delete/' +  index,{
                    method: 'DELETE',
                    headers: {
    ...
    const sendOnClick = async() => {
        if(isEditing){
             await fetch(apiURL +'/languages/update',{
                method: 'PUT',
            ...
            await fetch(apiURL +'/languages/new',{
                method: 'POST',
                headers: { 'Content-Type': 'application/json',
                'Authorization': 'Bearer '+ token },
                body: JSON.stringify({
    ...
    return(
        <div className="accordeon-content-inside row container">
            <div className="col-4">
                <TextField onClick={() => {setResultShown(false);}} size="small" id="outlined-basic" label="Language" variant="outlined"
