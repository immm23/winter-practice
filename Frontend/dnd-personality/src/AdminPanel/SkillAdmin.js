...
const columns = [
    ...
    {field: 'name', headerName: 'Skill', width: 150 }
];
...
const removeOnClick = async() => {
        if(selectionModel.length > 0){
            selectionModel.forEach(index => {
                fetch(apiURL + '/skills/delete/' +  index,{
                    method: 'DELETE',
                    headers: {
    ...
    const sendOnClick = async() => {
        if(isEditing){
            await fetch(apiURL +'/skills/update',{
                method: 'PUT',
    ...
    const handleEditButton = (event) => {
        if(selectionModel.length == 1){
            let filtered = skills.filter(function(x) {return x.id == selectionModel})
    ...
    return(
        <div className="accordeon-content-inside row container">
            <div className="col-4">
                <TextField onClick={() => {setResultShown(false);}} size="small" id="outlined-basic" label="Skill" variant="outlined"
